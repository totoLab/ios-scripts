...
def valid_config(config):
    ...
    if missing_options := [option for option in minimum_config_options if option not in config.keys()]:
        raise RuntimeError(
            f"Configuration is missing some fields: {missing_options}")
    return True
...
def check_chosen_device(available_devices):
    ...
    if len(args) < 2:
        if len(available_devices) > 1:
            raise RuntimeError(
                "No option was given as argument to the command. Retry.")
        else:
        ...
        if choice not in available_devices:
            raise RuntimeError(
                f"{choice} is not an available device, if you have the logs, specify in the config file how to reach them.")
